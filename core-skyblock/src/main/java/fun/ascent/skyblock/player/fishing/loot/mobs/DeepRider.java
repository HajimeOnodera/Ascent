...
public class DeepRider extends SkyblockMobEntity {

    public DeepRider() {
        super(EntityType.STRAY);
        setCustomName(text("<aqua>[Lv20] Deep Rider"));
    ...
    public @NonNull List<MobCategory> categories() {
        return List.of(MobCategory.AQUATIC, MobCategory.SKELETAL);
    }

    @Override
    public @NotNull List<GoalSelector> buildGoals() {
        return List.of(new MeleeChaseGoal(this, 1.7, 20));
    }
    ...
    public @Nullable DropTable dropTable() {
        return new DropTable() {
            ...
            public List<MobDrop> drops() {
                return List.of(
                        new MobDrop(ItemStack.of(Material.BONE), 100.0, 1, 4),
                        new MobDrop(ItemStack.of(Material.PRISMARINE_SHARD), 60.0, 1, 2)
                );
            }
        };
    }
